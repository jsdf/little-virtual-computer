...
function parseProgramText(programText) {
  // We'll use a regexp to find each token in the program text. Our assembly
  // language is very simple, so a pattern which matches any consecutive
  // sequence of non-whitespace characters is sufficient to find our tokens.
  const TOKEN_PATTERN = /\S+/;

  const tokens = [];
  let currentPosition = 0;
  while (currentPosition < programText.length) {
    // find next token  in our program text
    const currentMatch = programText.substring(currentPosition)
      .match(TOKEN_PATTERN);
    ...
    const tokenText = currentMatch[0];

    if (tokenText in instructions) {

    }

    // advance parser current position past the matched text
    currentPosition += currentMatch.index + tokenText.length;
...
function step() {
  ...
  instructions[instructionName].implementation();
}
